...
from config import SEEDS, ERROR_CODES, INITIAL_SLEEP_DURATION, ERRORS_TO_KEEP_PERCENTAGE, THREADS, MAX_WORKERS
...
if preprocessed:
    ...
    list_columns = ['urls']
    urls_by_publisher[list_columns] = urls_by_publisher[list_columns].map(convert_to_list)
else:
    # Load and preprocess data
...
async def main(seed):
    ...
    async with httpx.AsyncClient(limits=httpx.Limits(max_connections=750), verify=False) as client:
        sample_results = []
        for publisher, url in sample_urls:
            response = await async_request_publisher_data(client, [url], rate_limit)
            sample_results.append({'publisher': publisher, 'url': url, 'response': str(response)})
...
async def analyze_and_save(sample_results, filename):
    ...
    while retry_needed:
        ...
        urls_with_errors = sample_df[sample_df['response_code'].isin(error_codes)]['url']
        ...
        if len(urls_with_errors) > errors_to_keep:
            if THREADS:
                with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                    ...
                    futures = [executor.submit(fetch_data, url) for url in urls_with_errors]
            ...
            for _, row in retry_responses.iterrows():
                sample_df.loc[sample_df['url'] == row['url'], 'response'] = str(row['response'])
        ...
        print(f"Sleeping for {initial_sleep_duration} seconds before the next iteration...")
